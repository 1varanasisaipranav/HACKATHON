...
def update():
    ...
    if run:
        ...
        for m in range(num):
            ...
            if pipes_top[m].x < pig.x and pipes_top[m].score_tag:
                score += 1
                text.y = 1
        ...
        # Collision
        hit_info = pig.intersects()
        if hit_info.hit:
            run = False
            invoke(Func(pig.shake, duration =2))
            invoke(Func(pig.fade_out, duration =3))
            invoke(crash, delay = 3)

def crash():
    Text(text=f'You scored {score}! Reload  Game!',origin=(0,1), scale=3,color=color.red)
